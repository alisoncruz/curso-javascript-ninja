(function () {
  ...
  */
  ...
  var formatCpf = function (match, a1, a2, a3, a4) {
    return a1 + '.' + a2 + '.' + a3 + '-' + a4;
  };
  var regexCpf = /(\d{3})(\d{3})(\d{3})(\d{2})/g;

  var cpfFormatado1 = cleanCPF('049-214 3421-1').replace(regexCpf, formatCpf);
  var cpfFormatado2 = cleanCPF('210.458.522-05').replace(regexCpf, formatCpf);
  var cpfFormatado3 = cleanCPF('735 500 794 - 22').replace(regexCpf, formatCpf);
  var cpfFormatado4 = cleanCPF('101.123-131x32').replace(regexCpf, formatCpf);
  ...
  */
  ...
  var result = 'Os meses de janeiro, junho e julho começam com a letra j'.match(/ju\w{3}/g);
  console.log(result);
  ...
  */
  ...
  var emptyTags = '<div><ul><li></li><li></li><li><span></span></li></ul></div>'.match(/<\w+><\/\w+>/g);
  ...
  */
  ...
  var regexTag = /(<\w+>)([\w+\sà-ù]+)(<\/\w+>)/g

  var formatText = function (match, a1, a2, a3) {
    return a1 + 'O texto dentro da tag ' + a1.replace(/[<>]/g, '"') + ' é ' + '"' + a2 + '"' + a3 + '\n';
  }

  var replacedMarkupText = markup.replace(regexTag, formatText);

  console.log(replacedMarkupText);
